...
SERVICE_NAME="server-monitor-metrics"
SERVICE_FILE="/etc/systemd/system/${SERVICE_NAME}.service"
PY_FILE="${APP_DIR}/metrics_receiver.py"
...
[Unit]
Description=Server Monitor Metrics Receiver
...
Environment="DATABASE_PATH=${APP_DIR}/servers.db"
Environment="METRICS_PORT=8765"
...
echo "✅ Metrics 接收服务已安装并启动"
echo "📌 状态：systemctl status ${SERVICE_NAME} --no-pager -l"
echo "📌 日志：journalctl -u ${SERVICE_NAME} -f"
echo "📌 测试：curl -s http://127.0.0.1:8765/health"
