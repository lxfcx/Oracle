...
async async function copyText(id){let t=document.getElementById(id);if(!t)return;let txt=t.innerText||t.textContent||'';let ok=false;try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(txt);ok=true}}catch(e){}if(!ok){try{let a=document.createElement('textarea');a.value=txt;a.style.position='fixed';a.style.left='-9999px';document.body.appendChild(a);a.focus();a.select();ok=document.execCommand('copy');document.body.removeChild(a)}catch(e){}}let o=document.getElementById(id+'ok');if(o){o.textContent=ok?'✅ 已复制成功':'⚠️ 请手动复制';setTimeout(()=>o.textContent='',2200)}showToast(ok?'✅ 探针命令已复制成功':'⚠️ 自动复制失败，请手动选择命令复制')}function showToast(msg){let x=document.getElementById('toast');if(!x){x=document.createElement('div');x.id='toast';x.className='toast';document.body.appendChild(x)}x.textContent=msg;x.classList.add('show');setTimeout(()=>x.classList.remove('show'),2300)}
document.addEventListener('DOMContentLoaded',()=>{applyTheme();initView();live();setInterval(live,10000)});async function live(){try{let j=await(await fetch('/api/servers-live')).json();(j.servers||[]).forEach(s=>{['cpu','mem','disk'].forEach(k=>{document.querySelectorAll('[data-'+k+'="'+s.id+'"]').forEach(e=>{let v=s[k]||0;e.style.width=v+'%';let limit=Number(e.dataset.limit||90);e.classList.toggle('bad',v>=limit);e.classList.toggle('warn',v>=70&&v<limit)});});document.querySelectorAll('[data-uptime="'+s.id+'"]').forEach(e=>e.textContent=s.uptime);document.querySelectorAll('[data-status="'+s.id+'"]').forEach(e=>e.textContent=(s.online?'🟢 ':'🔴 ')+s.status);});}catch(e){}}
...
app.jinja_env.globals.update(fmt=fmt,dur=dur,duration=dur,exptext=exptext,expire_text=exptext,pricet=pricet,price_text=pricet,cycle=cycle,cycle_cn=cycle,fresh=fresh,flag=flag,server_flag=server_flag,fmt_size=fmt,age=age,server_location_cn=server_location_cn,bar_class=bar_class)
