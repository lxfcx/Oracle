...
def all_servers():
    ...
    for s in rows:
        ...
        s.update({'status':st,'metrics':m,'online':st.get('last_status')=='online','probe_fresh':fresh(m),'flag':server_flag(s),'location':' '.join([x for x in [s.get('country'),s.get('region'),s.get('city')] if x]) or '未知','expire_label':exptext(s.get('expire_at'),s.get('free_forever')),'price_label':pricet(s)})
        out.append(s)
    return out
def summary():
...
def server_detail(sid):
    ...
    s.update({'status':get_status(sid),'metrics':get_metrics(sid),'states':states(sid),'flag':server_flag(s),'location':' '.join([x for x in [s.get('country'),s.get('region'),s.get('city')] if x]) or '未知'}); s['agent_cmd']=agent_cmd(s)
...
def settings_page():
    ...
    return render('settings',SETTINGS,web_user=WEB_USERNAME,bot_token=BOT_TOKEN,admin_ids=','.join(ADMIN_IDS) if isinstance(ADMIN_IDS,list) else os.getenv('ADMIN_IDS',''),has_bg=bool(theme_bg_exists()))
...
BASE='''<!doctype html><html lang=zh-CN><head><meta charset=utf-8><script>(function(){let t=localStorage.getItem('theme')||'dark',g=localStorage.getItem('glass')||'glass';document.documentElement.classList.toggle('light',t==='light');document.documentElement.classList.toggle('solid',g==='solid')})();</script><meta name=viewport content="width=device-width,initial-scale=1"><title>Server Monitor Web</title><style>
...
app.jinja_env.globals.update(fmt=fmt,dur=dur,duration=dur,exptext=exptext,expire_text=exptext,pricet=pricet,price_text=pricet,cycle=cycle,cycle_cn=cycle,fresh=fresh,flag=flag,server_flag=server_flag,fmt_size=fmt,age=age)
