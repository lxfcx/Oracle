...
LOGIN='''<!doctype html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>登录</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 20% 20%,#173b67,#07111f 38%),#07111f;color:#edf5ff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif}.card{width:min(440px,92vw);padding:34px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);border-radius:28px;box-shadow:0 30px 90px #0008;backdrop-filter:blur(18px)}h1{margin:0 0 8px;font-size:30px}p{color:#9fb0c7}input,button{width:100%;padding:15px;border-radius:16px;margin:9px 0;border:1px solid rgba(255,255,255,.14);font-size:16px}input{background:#0004;color:#fff}button{background:linear-gradient(135deg,#6ee7ff,#a78bfa);color:#07111f;font-weight:900;border:0}.flash{background:#fb718522;border:1px solid #fb718577;padding:12px;border-radius:14px}</style></head><body><form class=card method=post><h1>🛡️✨ Web 面板登录</h1><p>Server Monitor Dashboard</p>{% with messages=get_flashed_messages(with_categories=true) %}{% for c,m in messages %}<div class=flash>{{m}}</div>{% endfor %}{% endwith %}<input name=username placeholder=账号 required><input name=password type=password placeholder=密码 required><button>🚀 登录控制台</button></form></body></html>'''
BASE='''<!doctype html><html lang=zh-CN><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>Server Monitor Web</title><style>:root{--bg:#07111f;--card:#ffffff14;--card2:#ffffff1f;--line:#ffffff22;--text:#edf5ff;--muted:#9fb0c7;--blue:#6ee7ff;--purple:#a78bfa;--green:#34d399;--red:#fb7185;--yellow:#fbbf24}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 15% 8%,#1d4e8966,transparent 32%),radial-gradient(circle at 95% 0,#7c3aed44,transparent 35%),var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif}a{color:inherit;text-decoration:none}.layout{display:grid;grid-template-columns:260px 1fr;min-height:100vh}.side{position:sticky;top:0;height:100vh;padding:22px;background:#040a14b8;border-right:1px solid var(--line);backdrop-filter:blur(18px)}.brand{display:flex;gap:12px;align-items:center;margin-bottom:26px}.brand .ico{width:48px;height:48px;border-radius:18px;display:grid;place-items:center;font-size:25px;background:linear-gradient(135deg,var(--blue),var(--purple))}.brand span,.muted,.small{color:var(--muted)}.nav a{display:flex;gap:10px;padding:12px;margin:8px 0;border:1px solid transparent;border-radius:16px}.nav a.active,.nav a:hover{background:var(--card2);border-color:var(--line)}.main{padding:24px 28px 60px;max-width:1600px;width:100%;margin:auto}.top{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:20px}.top h1{margin:0;font-size:30px}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}.grid2{display:grid;grid-template-columns:repeat(2,1fr);gap:16px}.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.card{border:1px solid var(--line);background:linear-gradient(180deg,var(--card2),var(--card));border-radius:24px;padding:20px;box-shadow:0 20px 60px #0003}.kpi .value{font-size:34px;font-weight:900}.ok{color:var(--green)}.bad{color:var(--red)}.warn{color:var(--yellow)}.badge{display:inline-flex;border:1px solid var(--line);border-radius:999px;padding:6px 10px;background:#ffffff14;font-size:13px}.table{width:100%;border-collapse:separate;border-spacing:0 10px}.table th{text-align:left;color:var(--muted);padding:0 12px}.table td{padding:14px 12px;background:#ffffff0f;border-top:1px solid var(--line);border-bottom:1px solid var(--line);vertical-align:middle}.table td:first-child{border-left:1px solid var(--line);border-radius:16px 0 0 16px}.table td:last-child{border-right:1px solid var(--line);border-radius:0 16px 16px 0}.progress{height:10px;background:#ffffff18;border-radius:99px;overflow:hidden;margin-top:7px}.bar{height:100%;background:linear-gradient(90deg,var(--green),var(--blue));border-radius:99px}.bar.warn{background:linear-gradient(90deg,var(--yellow),#fb923c)}.bar.bad{background:linear-gradient(90deg,#fb923c,var(--red))}.btns{display:flex;gap:8px;flex-wrap:wrap}.btn,button{border:1px solid var(--line);background:#ffffff14;color:#eaf2ff;border-radius:13px;padding:9px 12px;font-weight:800;cursor:pointer}.btn.primary,button.primary{background:linear-gradient(135deg,var(--blue),var(--purple));color:#07111f;border:0}.btn.danger,button.danger{background:#fb718522;border-color:#fb718577;color:#fecdd3}input,select,textarea{width:100%;padding:12px;border-radius:14px;border:1px solid var(--line);background:#0004;color:#fff}label{display:block;margin:13px 0 7px;color:#cfe0f5;font-weight:800}.formgrid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px 16px}.flash{padding:12px 14px;border-radius:16px;margin-bottom:14px;border:1px solid var(--line);background:#ffffff14}.flash.success{background:#34d39922;border-color:#34d39966}.flash.error{background:#fb718522;border-color:#fb718577}pre{white-space:pre-wrap;word-break:break-all;background:#0005;border:1px solid var(--line);padding:16px;border-radius:18px}.small{font-size:13px;line-height:1.65}hr{border:0;border-top:1px solid var(--line);margin:18px 0}@media(max-width:1100px){.layout{grid-template-columns:1fr}.side{height:auto;position:relative}.nav{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.nav a{margin:0}.grid{grid-template-columns:repeat(2,1fr)}.grid2,.grid3,.formgrid{grid-template-columns:1fr}}@media(max-width:640px){.main{padding:16px}.grid{grid-template-columns:1fr}.top{display:block}}</style><script>async function r(){try{let j=await(await fetch('/api/summary')).json();for(let k of ['total','online','offline','probes','expiring','expired']){let e=document.querySelector('[data-kpi="'+k+'"]');if(e)e.textContent=j[k]}let t=document.querySelector('[data-now]');if(t)t.textContent=j.time}catch(e){}}setInterval(r,10000);function delok(){return confirm('确认删除服务器？')}</script></head><body><div class=layout><aside class=side><div class=brand><div class=ico>🛡️</div><div><b>Server Monitor</b><br><span>Web Dashboard</span></div></div><nav class=nav><a class="{{'active' if active=='dashboard' else ''}}" href="/">📊 总览大屏</a><a class="{{'active' if active=='servers' else ''}}" href="/servers">🖥️ 服务器</a><a class="{{'active' if active=='add' else ''}}" href="/servers/add">➕ 添加服务器</a><a class="{{'active' if active=='local' else ''}}" href="/local">🏠 本机</a><a class="{{'active' if active=='events' else ''}}" href="/events">🧾 事件</a><a href="/logout">🚪 退出</a></nav><div class=small style="margin-top:22px">👤 {{username}}<br>🕒 <span data-now>{{now}}</span></div></aside><main class=main>{% with messages=get_flashed_messages(with_categories=true) %}{% for c,m in messages %}<div class="flash {{c}}">{{m}}</div>{% endfor %}{% endwith %}{{body|safe}}</main></div></body></html>'''
DASH='''<div class=top><h1>📊✨ 服务器总览大屏</h1><div class=btns><a class="btn primary" href="/servers/add">➕ 添加服务器</a></div></div><div class=grid><div class="card kpi"><div>📦 总数</div><div class=value data-kpi=total>{{data.total}}</div></div><div class="card kpi"><div>🟢 在线</div><div class="value ok" data-kpi=online>{{data.online}}</div></div><div class="card kpi"><div>🔴 离线</div><div class="value bad" data-kpi=offline>{{data.offline}}</div></div><div class="card kpi"><div>📡 探针在线</div><div class=value data-kpi=probes>{{data.probes}}</div></div></div><div class=grid2 style="margin-top:16px"><div class=card><h2>🏠 本机状态</h2><p><span class=badge>🌐 {{local.public_ip or '未知'}}</span> <span class=badge>🧩 {{local.cpu_count or 0}} 核</span> <span class=badge>⏱️ {{local.uptime or '未知'}}</span></p><hr>🔥 CPU {{'%.0f'|format(local.cpu or 0)}}%<div class=progress><div class="bar {{'bad' if (local.cpu or 0)>=90 else 'warn' if (local.cpu or 0)>=80 else ''}}" style="width:{{local.cpu or 0}}%"></div></div><br>🧠 内存 {{fmt(local.mem_used or 0)}} / {{fmt(local.mem_total or 0)}} ({{'%.0f'|format(local.mem_percent or 0)}}%)<div class=progress><div class=bar style="width:{{local.mem_percent or 0}}%"></div></div><br>💾 磁盘 {{fmt(local.disk_used or 0)}} / {{fmt(local.disk_total or 0)}} ({{'%.0f'|format(local.disk_percent or 0)}}%)<div class=progress><div class=bar style="width:{{local.disk_percent or 0}}%"></div></div></div><div class=card><h2>⏰ 到期和风险</h2><div class=grid3><div class=card><div>⚠️ 7天内到期</div><div class="value warn" data-kpi=expiring>{{data.expiring}}</div></div><div class=card><div>🚨 已过期</div><div class="value bad" data-kpi=expired>{{data.expired}}</div></div><div class=card><div>⚪ 未知</div><div class=value data-kpi=unknown>{{data.unknown}}</div></div></div><p class=small>支持探针真实 uptime、CPU/内存/硬盘/流量、阈值告警、续费到期、事件统计。</p></div></div><div class=card style="margin-top:16px"><h2>🖥️ 服务器面板</h2><table class=table><thead><tr><th>服务器</th><th>状态</th><th>配置</th><th>资源</th><th>续费</th><th>操作</th></tr></thead><tbody>{% for s in data.servers %}{% set m=s.metrics %}<tr><td><b>{{'🟢' if s.online else '🔴'}} {{s.flag}} {{s.name}}</b><br><span class=muted>ID{{s.id}}｜{{s.host}}:{{s.check_port}}｜{{s.location}}</span></td><td><span class=badge>{{'🟢 在线' if s.online else '🔴 离线'}}</span><br><span class=small>探针：{{'🟢 在线' if s.probe_fresh else '🟠 超时/未上报'}}</span></td><td>🧩 {{m.cpu_cores or '?'}}C<br>🧠 {{fmt(m.mem_total or 0) if m.mem_total else '未知'}}<br>💾 {{fmt(m.disk_total or 0) if m.disk_total else '未知'}}</td><td>🔥 {{'%.0f'|format(m.cpu_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.cpu_percent or 0)>=(s.cpu_alert or 90) else ''}}" style="width:{{m.cpu_percent or 0}}%"></div></div>🧠 {{'%.0f'|format(m.mem_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.mem_percent or 0)>=(s.mem_alert or 90) else ''}}" style="width:{{m.mem_percent or 0}}%"></div></div>💾 {{'%.0f'|format(m.disk_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.disk_percent or 0)>=(s.disk_alert or 90) else ''}}" style="width:{{m.disk_percent or 0}}%"></div></div></td><td>{{s.expire_label}}<br>{{s.price_label}}</td><td><a class=btn href="/servers/{{s.id}}">详情</a></td></tr>{% else %}<tr><td colspan=6>📭 暂无服务器</td></tr>{% endfor %}</tbody></table></div><div class=card style="margin-top:16px"><h2>🧾 最新事件</h2><table class=table>{% for e in events %}<tr><td><b>{{e.title}}</b><br><span class=muted>{{e.created_at}}｜{{e.event_type}}</span></td><td>{{e.content}}</td></tr>{% else %}<tr><td>暂无事件</td></tr>{% endfor %}</table></div>'''
...
EVENTS='''<div class=top><h1>🧾✨ 事件记录</h1><a class=btn href="/">📊 总览</a></div><div class=card><table class=table><thead><tr><th>时间</th><th>类型</th><th>标题</th><th>内容</th></tr></thead><tbody>{% for e in events %}<tr><td>{{e.created_at}}</td><td><span class=badge>{{e.event_type}}</span></td><td><b>{{e.title}}</b></td><td>{{e.content}}</td></tr>{% else %}<tr><td colspan=4>暂无事件</td></tr>{% endfor %}</tbody></table></div>'''
