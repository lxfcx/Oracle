...
.servercard h3{margin:0 0 8px}.servercard .meta{display:flex;gap:6px;flex-wrap:wrap;margin:8px 0}.copyok{color:var(--green);font-size:13px;margin-left:8px}
...
async function copyText(id){let t=document.getElementById(id);if(!t)return;await navigator.clipboard.writeText(t.innerText||t.textContent);let o=document.getElementById(id+'ok');if(o){o.textContent='✅ 已复制';setTimeout(()=>o.textContent='',1600)}}
document.addEventListener('DOMContentLoaded',()=>{applyTheme();initView()});
...
</script></head><body><div class=layout><aside class=side><div class=brand><div class=ico>🛡️</div><div><b>Server Monitor</b><span>Star Glass Dashboard</span></div></div><div class=switches><button class=themebtn onclick="toggleTheme()" type=button><span id=themeText>☀️ 日间明亮</span></button><button class=themebtn onclick="toggleGlass()" type=button><span id=glassText>⬛ 实色背景</span></button></div><nav class=nav><a class="{{'active' if active=='dashboard' else ''}}" href="/">📊 总览大屏</a><a class="{{'active' if active=='servers' else ''}}" href="/servers">🖥️ 服务器</a><a class="{{'active' if active=='add' else ''}}" href="/servers/add">➕ 添加服务器</a><a class="{{'active' if active=='local' else ''}}" href="/local">🏠 本机</a><a class="{{'active' if active=='events' else ''}}" href="/events">🧾 事件</a><a href="/logout">🚪 退出</a></nav><div class=small style="margin-top:22px">👤 {{username}}<br>🕒 <span data-now>{{now}}</span><br>🌌 星空 · 🧊 玻璃 · 🇺🇳 国旗</div></aside><main class=main>{% with messages=get_flashed_messages(with_categories=true) %}{% for c,m in messages %}<div class="flash {{c}}">{{m}}</div>{% endfor %}{% endwith %}{{body|safe}}</main></div></body></html>'''
DASH='''<div class=top><h1>📊✨ 服务器总览大屏</h1><div class=btns><button onclick="setView('card')">🔳 卡片视图</button><button onclick="setView('table')">📋 表格视图</button><a class="btn primary" href="/servers/add">➕ 添加服务器</a></div></div><div class=grid><div class="card kpi"><div class=label>📦 总数</div><div class=value data-kpi=total>{{data.total}}</div></div><div class="card kpi"><div class=label>🟢 在线</div><div class="value ok" data-kpi=online>{{data.online}}</div></div><div class="card kpi"><div class=label>🔴 离线</div><div class="value bad" data-kpi=offline>{{data.offline}}</div></div><div class="card kpi"><div class=label>📡 探针在线</div><div class=value data-kpi=probes>{{data.probes}}</div></div></div><div class=grid2 style="margin-top:16px"><div class=card><h2>🏠 本机状态</h2><p><span class=badge>🌐 {{local.public_ip or '未知'}}</span> <span class=badge>🧩 {{local.cpu_count or 0}} 核</span> <span class=badge>⏱️ {{local.uptime or '未知'}}</span></p><hr>🔥 CPU {{'%.0f'|format(local.cpu or 0)}}%<div class=progress><div class="bar {{'bad' if (local.cpu or 0)>=90 else 'warn' if (local.cpu or 0)>=80 else ''}}" style="width:{{local.cpu or 0}}%"></div></div><br>🧠 内存 {{fmt_size(local.mem_used or 0)}} / {{fmt_size(local.mem_total or 0)}} ({{'%.0f'|format(local.mem_percent or 0)}}%)<div class=progress><div class=bar style="width:{{local.mem_percent or 0}}%"></div></div><br>💾 磁盘 {{fmt_size(local.disk_used or 0)}} / {{fmt_size(local.disk_total or 0)}} ({{'%.0f'|format(local.disk_percent or 0)}}%)<div class=progress><div class=bar style="width:{{local.disk_percent or 0}}%"></div></div></div><div class=card><h2>⏰ 到期和风险</h2><div class=grid3><div class=card><div class=label>⚠️ 7天内到期</div><div class="value warn" data-kpi=expiring>{{data.expiring}}</div></div><div class=card><div class=label>🚨 已过期</div><div class="value bad" data-kpi=expired>{{data.expired}}</div></div><div class=card><div class=label>⚪ 未知</div><div class=value data-kpi=unknown>{{data.unknown}}</div></div></div><p class=small>支持探针真实 uptime、CPU/内存/硬盘/流量、阈值告警、续费到期、事件统计。</p></div></div><div class=card style="margin-top:16px"><h2>🖥️ 服务器面板</h2><div data-view-card class=cardgrid>{% for s in data.servers %}{% set m=s.metrics %}<div class="card servercard"><h3>{{'🟢' if s.online else '🔴'}} {{s.flag or flag(s.country_code)}} {{s.name}}</h3><div class=meta><span class=badge>ID{{s.id}}</span><span class=badge>{{s.host}}:{{s.check_port}}</span><span class=badge>{{s.flag or flag(s.country_code)}} {{s.location}}</span></div><p class=small>📡 探针：{{'🟢 在线' if s.probe_fresh else '🟠 超时/未上报'}}｜{{s.expire_label}}｜{{s.price_label}}</p><div class=grid3><div>🧩<br><b>{{m.cpu_cores or '?' }}C</b></div><div>🧠<br><b>{{fmt_size(m.mem_total or 0) if m.mem_total else '未知'}}</b></div><div>💾<br><b>{{fmt_size(m.disk_total or 0) if m.disk_total else '未知'}}</b></div></div><hr>🔥 CPU {{'%.0f'|format(m.cpu_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.cpu_percent or 0)>=(s.cpu_alert or 90) else ''}}" style="width:{{m.cpu_percent or 0}}%"></div></div>🧠 内存 {{'%.0f'|format(m.mem_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.mem_percent or 0)>=(s.mem_alert or 90) else ''}}" style="width:{{m.mem_percent or 0}}%"></div></div>💾 硬盘 {{'%.0f'|format(m.disk_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.disk_percent or 0)>=(s.disk_alert or 90) else ''}}" style="width:{{m.disk_percent or 0}}%"></div></div><br><a class=btn href="/servers/{{s.id}}">详情</a></div>{% else %}<div class=card>📭 暂无服务器</div>{% endfor %}</div><div data-view-table class=hidden><table class=table><thead><tr><th>服务器</th><th>状态</th><th>配置</th><th>资源</th><th>续费</th><th>操作</th></tr></thead><tbody>{% for s in data.servers %}{% set m=s.metrics %}<tr><td><b>{{'🟢' if s.online else '🔴'}} {{s.flag or flag(s.country_code)}} {{s.name}}</b><br><span class=muted>ID{{s.id}}｜{{s.host}}:{{s.check_port}}｜{{s.flag or flag(s.country_code)}} {{s.location}}</span></td><td><span class=badge>{{'🟢 在线' if s.online else '🔴 离线'}}</span><br><span class=small>探针：{{'🟢 在线' if s.probe_fresh else '🟠 超时/未上报'}}</span></td><td>🧩 {{m.cpu_cores or '?'}}C<br>🧠 {{fmt_size(m.mem_total or 0) if m.mem_total else '未知'}}<br>💾 {{fmt_size(m.disk_total or 0) if m.disk_total else '未知'}}</td><td>🔥 {{'%.0f'|format(m.cpu_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.cpu_percent or 0)>=(s.cpu_alert or 90) else ''}}" style="width:{{m.cpu_percent or 0}}%"></div></div>🧠 {{'%.0f'|format(m.mem_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.mem_percent or 0)>=(s.mem_alert or 90) else ''}}" style="width:{{m.mem_percent or 0}}%"></div></div>💾 {{'%.0f'|format(m.disk_percent or 0)}}%<div class=progress><div class="bar {{'bad' if (m.disk_percent or 0)>=(s.disk_alert or 90) else ''}}" style="width:{{m.disk_percent or 0}}%"></div></div></td><td>{{s.expire_label}}<br>{{s.price_label}}</td><td><a class=btn href="/servers/{{s.id}}">详情</a></td></tr>{% else %}<tr><td colspan=6>📭 暂无服务器</td></tr>{% endfor %}</tbody></table></div></div><div class=card style="margin-top:16px"><h2>🧾 最新事件</h2><div class="scrollbox compact"><table class=table>{% for e in events %}<tr><td><b>{{e.title}}</b><br><span class=muted>{{e.created_at}}｜{{e.event_type}}</span></td><td>{{e.content}}</td></tr>{% else %}<tr><td>暂无事件</td></tr>{% endfor %}</table></div></div>'''
SERVERS='''<div class=top><h1>🖥️✨ 服务器管理</h1><div class=btns><button onclick="setView('card')">🔳 卡片视图</button><button onclick="setView('table')">📋 表格视图</button><a class="btn primary" href="/servers/add">➕ 添加服务器</a><a class=btn href="/">📊 总览</a></div></div><div class=grid><div class="card kpi"><div class=label>📦 总数</div><div class=value>{{data.total}}</div></div><div class="card kpi"><div class=label>🟢 在线</div><div class="value ok">{{data.online}}</div></div><div class="card kpi"><div class=label>🔴 离线</div><div class="value bad">{{data.offline}}</div></div><div class="card kpi"><div class=label>📡 探针</div><div class=value>{{data.probes}}</div></div></div><div class=card style="margin-top:16px"><div data-view-card class=cardgrid>{% for s in data.servers %}{% set m=s.metrics %}<div class="card servercard"><h3>{{'🟢' if s.online else '🔴'}} {{s.flag or flag(s.country_code)}} {{s.name}}</h3><div class=meta><span class=badge>#{{s.id}}</span><span class=badge>{{s.host}}:{{s.check_port}}</span><span class=badge>{{s.flag or flag(s.country_code)}} {{s.location}}</span></div><p class=small>{{s.note or '无备注'}}<br>📡 {{'🟢 在线' if s.probe_fresh else '🟠 未上报/超时'}}｜{{s.expire_label}}｜{{s.price_label}}</p><div>🧩 {{m.cpu_cores or '?'}}C ｜ 🧠 {{fmt_size(m.mem_total or 0) if m.mem_total else '?'}} ｜ 💾 {{fmt_size(m.disk_total or 0) if m.disk_total else '?'}}</div><hr>🎯 阈值：🔥{{'%.0f'|format(s.cpu_alert or 90)}}% ｜ 🧠{{'%.0f'|format(s.mem_alert or 90)}}% ｜ 💾{{'%.0f'|format(s.disk_alert or 90)}}%<br><br><div class=btns><a class="btn primary" href="/servers/{{s.id}}">查看</a><a class=btn href="/servers/{{s.id}}/edit">编辑</a></div></div>{% else %}<div class=card>📭 暂无服务器</div>{% endfor %}</div><div data-view-table class=hidden><table class=table><thead><tr><th>ID</th><th>服务器</th><th>探针/配置</th><th>阈值</th><th>续费</th><th>操作</th></tr></thead><tbody>{% for s in data.servers %}{% set m=s.metrics %}<tr><td><b>#{{s.id}}</b></td><td><b>{{'🟢' if s.online else '🔴'}} {{s.flag or flag(s.country_code)}} {{s.name}}</b><br><span class=muted>{{s.host}}:{{s.check_port}}｜{{s.flag or flag(s.country_code)}} {{s.location}}</span><br><span class=small>{{s.note or '无备注'}}</span></td><td>📡 {{'🟢 在线' if s.probe_fresh else '🟠 未上报/超时'}}<br>🧩 {{m.cpu_cores or '?'}}C｜🧠 {{fmt_size(m.mem_total or 0) if m.mem_total else '?'}}｜💾 {{fmt_size(m.disk_total or 0) if m.disk_total else '?'}}</td><td>🔥 {{'%.0f'|format(s.cpu_alert or 90)}}%<br>🧠 {{'%.0f'|format(s.mem_alert or 90)}}%<br>💾 {{'%.0f'|format(s.disk_alert or 90)}}%</td><td>{{s.expire_label}}<br>{{s.price_label}}</td><td><div class=btns><a class="btn primary" href="/servers/{{s.id}}">查看</a><a class=btn href="/servers/{{s.id}}/edit">编辑</a></div></td></tr>{% else %}<tr><td colspan=6>📭 暂无服务器</td></tr>{% endfor %}</tbody></table></div></div>'''
DETAIL='''<div class=top><h1>🖥️ {{s.flag or flag(s.country_code)}} {{s.name}}</h1><div class=btns><a class=btn href="/servers">📋 返回</a><a class="btn primary" href="/servers/{{s.id}}/edit">✏️ 编辑</a></div></div>{% set m=s.metrics %}<div class=grid3><div class="card kpi"><div class=label>📡 状态</div><div class="value {{'ok' if s.status.last_status=='online' else 'bad' if s.status.last_status=='offline' else ''}}">{{'🟢 在线' if s.status.last_status=='online' else '🔴 离线' if s.status.last_status=='offline' else '⚪ 未知'}}</div><div class=small>{{s.status.last_checked_at or '未知'}}</div></div><div class="card kpi"><div class=label>⏱️ 运行时长</div><div class=value>{{duration(m.uptime_seconds or 0)}}</div><div class=small>开机：{{m.boot_time or '未知'}}</div></div><div class="card kpi"><div class=label>⏰ 到期</div><div class=value style="font-size:22px">{{expire_text(s.expire_at,s.free_forever)}}</div><div class=small>{{price_text(s)}}｜{{cycle_cn(s.cycle)}}</div></div></div><div class=grid2 style="margin-top:16px"><div class=card><h2>⚙️ 服务器配置</h2><p><span class=badge>🆔 ID {{s.id}}</span> <span class=badge>{{s.flag or flag(s.country_code)}} {{s.location}}</span></p><p>🌐 主机：<code>{{s.host}}:{{s.check_port}}</code></p><p>🏢 运营商：{{s.isp or '未知'}}</p><p>🧬 系统：{{s.os_name or '未知系统'}}</p><p>📝 备注：{{s.note or '无'}}</p><hr><div class=grid3><div class=card>🧩 CPU<br><b>{{m.cpu_cores or '?'}} Cores</b></div><div class=card>🧠 内存<br><b>{{fmt_size(m.mem_total or 0) if m.mem_total else '未知'}}</b></div><div class=card>💾 硬盘<br><b>{{fmt_size(m.disk_total or 0) if m.disk_total else '未知'}}</b></div></div></div><div class=card><h2>📊 资源使用</h2>🔥 CPU {{'%.0f'|format(m.cpu_percent or 0)}}% / {{'%.0f'|format(s.cpu_alert or 90)}}%<div class=progress><div class="bar {{'bad' if (m.cpu_percent or 0)>=(s.cpu_alert or 90) else ''}}" style="width:{{m.cpu_percent or 0}}%"></div></div><br>🧠 内存 {{fmt_size(m.mem_used or 0) if m.mem_used else '未知'}} / {{fmt_size(m.mem_total or 0) if m.mem_total else '未知'}} ({{'%.0f'|format(m.mem_percent or 0)}}%) / {{'%.0f'|format(s.mem_alert or 90)}}%<div class=progress><div class="bar {{'bad' if (m.mem_percent or 0)>=(s.mem_alert or 90) else ''}}" style="width:{{m.mem_percent or 0}}%"></div></div><br>💾 硬盘 {{fmt_size(m.disk_used or 0) if m.disk_used else '未知'}} / {{fmt_size(m.disk_total or 0) if m.disk_total else '未知'}} ({{'%.0f'|format(m.disk_percent or 0)}}%) / {{'%.0f'|format(s.disk_alert or 90)}}%<div class=progress><div class="bar {{'bad' if (m.disk_percent or 0)>=(s.disk_alert or 90) else ''}}" style="width:{{m.disk_percent or 0}}%"></div></div><hr>🌐 流量：⬇️ {{fmt_size(m.rx_bytes or 0)}} / ⬆️ {{fmt_size(m.tx_bytes or 0)}}<br>📡 探针：{{'🟢 在线' if fresh(m) else '🟠 超时/未上报'}}｜{{m.updated_at or '未知'}}</div></div><div class=grid2 style="margin-top:16px"><div class=card><h2>📡 一键部署探针</h2><p class=small>复制到这台服务器 SSH 执行，探针静默上报，离线/恢复由主机器人统一推送。</p><pre id="agentcmd">{{s.agent_cmd}}</pre><button class=primary type=button onclick="copyText('agentcmd')">📋 复制探针命令</button><span id="agentcmdok" class=copyok></span></div><div class=card><h2>🛠️ 操作</h2><div class=btns><form method=post action="/servers/{{s.id}}/check"><button class=primary>📡 立即检测</button></form><form method=post action="/servers/{{s.id}}/refresh"><button>🌍 刷新地区</button></form><a class=btn href="/servers/{{s.id}}/edit">✏️ 编辑资料/阈值</a><form method=post action="/servers/{{s.id}}/delete" onsubmit="return delok()"><button class=danger>🗑️ 删除</button></form></div><hr><h3>🎯 告警状态</h3>{% for key,label in [('cpu','🔥 CPU'),('mem','🧠 内存'),('disk','💾 硬盘')] %}{% set st=s.states.get(key) %}<p>{{label}}：{{'🚨 告警中' if st and st.active else '✅ 正常'}}{% if st %}｜上次 {{st.last_value|round(0)}}%｜{{st.last_sent_at}}{% endif %}</p>{% endfor %}</div></div>'''
...
app.jinja_env.globals.update(fmt=fmt,dur=dur,exptext=exptext,pricet=pricet,cycle=cycle,fresh=fresh,flag=flag,fmt_size=fmt,expire_text=exptext,price_text=pricet)
